...
export const PrizeAssignmentFlow: React.FC<PrizeAssignmentFlowProps> = ({ eventId }) => {
  ...
  const loadData = async () => {
    try {
      ...
      // Get assigned prizes with recipient information
      const assignedPrizesData = allPrizesData
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Previously Assigned Prizes */}
      {assignedPrizes.length > 0 && (
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
            Previously Assigned Prizes ({assignedPrizes.length})
          </h3>
              ...
              >
                ...
                {prize.rank && (
                  <div className="flex-shrink-0">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${
                      prize.rank === 1 ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400' :
                      prize.rank === 2 ? 'bg-gray-100 dark:bg-gray-700/50 text-gray-700 dark:text-gray-300' :
                      prize.rank === 3 ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' :
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 truncate">
                    Assigned to: <span className="font-medium text-gray-900 dark:text-white">
                      {prize.recipient?.full_name || 'Unknown'}
                    </span>
                  </p>
                  {prize.price && (
                ...
                <div className="flex-shrink-0">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    ...
                    {prize.status === 'fulfilled' ? 'Fulfilled' : 'Assigned'}
                  </span>
                </div>
              </div>
            ))}
